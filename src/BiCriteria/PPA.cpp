...
PPA::PPA(const AdjacencyMatrix &adj_matrix, Pair<double> eps, const LoggerPtr logger) :
    BOAStar(adj_matrix, {eps[0], eps[1]}, logger), eps_pair(eps) {
...
void PPA::insert(PathPairPtr &pp, PPQueue &queue) {
    ...
    std::vector<PathPairPtr> &closed_pps = expanded[pp->id];
    for (auto existing_pp = closed_pps.begin(); existing_pp != closed_pps.end(); ++existing_pp) {
        if (pp->if_merge_bounded(*existing_pp, this->eps_pair)){
            num_missed_merge ++;
            break;
        }
    }
...
void PPA::operator()(size_t source, size_t target, Heuristic &heuristic, SolutionSet &solutions, unsigned int time_limit) {
 ...
 continue;
    ...
    std::cout << "missed merge: " << num_missed_merge << std::endl;
