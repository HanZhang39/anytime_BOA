...
void IterativePPA::operator()(size_t source, size_t target, Heuristic &heuristic, SolutionSet &solutions, unsigned int time_limit){
    num_expansion = 0;
    num_generation = 0;
    SolutionSet all_solutions;
    ...
    for (auto eps:eps_list){
        ...
        while (open.empty() == false) {
            ...
            if (pp->id == target) {
                ...
                if (pp_solutions.back()->bottom_right != last_log){
                    last_log = pp_solutions.back()->bottom_right ;
                    log_solution(last_log);
                }
                continue;
            }
        ...
        for (auto solution = pp_solutions.begin(); solution != pp_solutions.end(); ++solution) {
            all_solutions.push_back((*solution)->bottom_right);
        }
    ...
    this->end_logging(all_solutions);



}

void IterativePPA::init_search(){
    expanded.clear();
