...
bool is_dominating(NodePtr a, NodePtr b){
...
std::ostream& operator<<(std::ostream &stream, const std::pair<std::clock_t, NodePtr> &time_node) {
  // Printed in JSON format
  auto node = time_node.second;
  ...
  stream
    ...
    <<      "\"time\": " << (double)time_node.first / CLOCKS_PER_SEC<< ", "
    <<      "\"full_cost\": " << node->f
    << "}";
  return stream;
}
...
bool PathPair::update_nodes_by_merge_if_bounded_keep_track(const PathPairPtr &other, const Pair<double> eps, std::list<NodePtr> & pruned_list) {
    ...
    if (!is_dominating(new_top_left, pruned_top_left) && !is_dominating(new_bottom_right, pruned_top_left)){
      pruned_list.push_back(pruned_top_left);
    }

    if (!is_dominating(new_top_left, pruned_bottom_right) && !is_dominating(new_bottom_right, pruned_bottom_right)){
